...
module.exports = class Service {
  ...
  putObjects() {
    const fnames = util.getFilenames(path.resolve(this.context, this.config.Filepath), this.config.Recursive, this.config.Exclude);
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        if (!fileConfig || typeof fileConfig !== 'object') {
          fileConfig = null;
          throw new Error('Error loading cos-config: should export an object.');
        }
      } catch (e) {
        console.log(e);
      }
